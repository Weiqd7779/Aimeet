  ...
  return (
    <section className={`panel flex min-h-[600px] flex-col ${dimmed ? "opacity-45" : ""}`}>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <p className="eyebrow">03 · 決策狀態</p>
          <h2 className="text-xl font-semibold text-white">可追溯的決策</h2>
        </div>
        ...
        {meeting.decisions.map((decision) => (
          <article key={decision.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
            <div className="flex items-start justify-between gap-2">
              ...
              <span className={`status-mini ${decision.status}`}>{({ candidate: "候選", confirmed: "已確認", rejected: "已否決" } as Record<string, string>)[decision.status] ?? decision.status}</span>
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
              <div><p className="label-green">支持理由</p><ul className="mt-1 space-y-1 text-slate-300">{decision.reasons_for.map((reason) => <li key={reason}>＋ {reason}</li>)}</ul></div>
              <div><p className="label-red">反對理由</p><ul className="mt-1 space-y-1 text-slate-300">{decision.reasons_against.map((reason) => <li key={reason}>− {reason}</li>)}</ul></div>
            </div>
            {decision.constraints.length > 0 && <div className="mt-3 border-t border-white/10 pt-3"><p className="label-muted">限制條件</p><p className="mt-1 text-xs text-slate-300">{decision.constraints.join(" · ")}</p></div>}
            {decision.conflicts.length > 0 && <div className="mt-3 flex flex-wrap gap-1">{decision.conflicts.map((id) => <span key={id} className="rounded-full bg-red-500/15 px-2 py-1 text-[10px] text-red-200">{alertById.get(id)?.title || "關聯衝突"}</span>)}</div>}
      ...
      <button className="button-primary mt-auto w-full justify-center" disabled={meeting.status === "idle" || meeting.status === "connecting"} onClick={meeting.generateReport}>產生報告</button>
    </section>
  );
}
