  ...
  return (
    <div className="fixed inset-0 z-20 flex justify-end bg-black/60 backdrop-blur-sm">
      <aside className="h-full w-full max-w-4xl overflow-y-auto border-l border-white/10 bg-[#0c1220] p-6 shadow-2xl">
        ...
        <div className="mt-6 flex gap-2 overflow-x-auto border-b border-white/10 pb-3">{[["facts", `Key Facts (${report.key_facts?.length ?? 0})`], ["table", "Decision Table"], ["mermaid", "Mermaid"], ["prd", "PRD"], ["work", "Work Items"]].map(([id, label]) => <button key={id} onClick={() => setTab(id)} className={`tab whitespace-nowrap ${tab === id ? "active" : ""}`}>{label}</button>)}</div>
