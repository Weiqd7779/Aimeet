...
export default function Home() {
  ...
  const statusLabel = { idle: "待機", connecting: "連線中", live: "進行中", ended: "已結束" };
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,_#12304a_0,_transparent_35%),#070b14] px-4 py-5 text-slate-200 sm:px-6 lg:px-8">
      <header className="mx-auto mb-5 flex max-w-[1800px] flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div><p className="eyebrow">AIMEET · 多模態會議助理</p><h1 className="text-xl font-bold tracking-tight text-white sm:text-2xl">即時決策助理</h1></div>
        </div>
        <div className="flex items-center gap-3">
          {meeting.mockMode && <span className="rounded-full border border-amber-300/30 bg-amber-300/10 px-2.5 py-1 text-[10px] font-bold tracking-widest text-amber-200">模擬模式</span>}
          <span className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-2.5 py-1 text-[10px] font-bold tracking-widest text-cyan-200">模型：{meeting.liveProvider.toUpperCase()}</span>
          ...
          <button className="flex items-center gap-2 text-xs text-slate-400 hover:text-cyan-200" onClick={() => setComparison(!comparison)}>{comparison ? <ToggleRight className="text-cyan-300" size={22} /> : <ToggleLeft size={22} />} 對照純逐字稿</button>
